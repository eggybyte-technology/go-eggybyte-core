...
func TestBootstrap_MultipleServices(t *testing.T) {
	...
	// Use BootstrapWithContext to allow cancellation
	err := BootstrapWithContext(ctx, cfg, svc1, svc2, svc3)

	// Should return nil on graceful shutdown due to context cancellation
	assert.NoError(t, err)
}
