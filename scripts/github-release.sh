...
# GitHub Release Script for EggyByte Core
# This script creates and pushes a new tag to GitHub
...
check_uncommitted_changes() {
    if ! git diff-index --quiet HEAD --; then
        ...
        git status --short
        echo
        read -p "Do you want to commit these changes before creating the tag? (y/N): " -n 1 -r
        echo
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            git add .
...
verify_tag() {
    log_info "Verifying tag creation..."
    ...
    if git ls-remote --tags origin | grep -q "refs/tags/$VERSION$"; then
        log_success "Tag $VERSION verified on GitHub"
    else
        log_warning "Tag verification failed, but it may still be processing"
    fi
    ...
    echo
    log_info "Next steps:"
    echo "  1. Visit: https://github.com/eggybyte-technology/go-eggybyte-core/releases"
    echo "  2. Check the Actions tab for automated release build"
    echo "  3. Edit the release description if needed"
...
# Main function
main() {
    log_info "Starting GitHub release process..."
    ...
    log_success "GitHub release process completed!"
}
